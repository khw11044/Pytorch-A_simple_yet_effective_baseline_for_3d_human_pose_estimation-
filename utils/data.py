...
class H36MDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for cam in cams:
            sample['confidences'][cam] = torch.Tensor(
                self.data['confidences'][cam][idx].astype('float32')).cuda()
...
class SkiDataset(Dataset):
    ...
    def __init__(self, fname, normalize_2d=True, subjects=[103, 110, 115, 124, 202, 207, 214, 221, 302, 309, 405, 412]):
        ...
        for cam in cams:
            ...
            self.data['image_path'][cam] = np.array(
                self.data['image_path'][cam])[selection_array]

            if normalize_2d:

                self.add_data['root_joint'][cam] = np.transpose(self.data['poses_2d_pred'][cam].reshape(
                    -1, 16, 2), (0, 2, 1))[:, :, [0]]               # (b,16,2) --> (b,2,16) --> (136399, 2, 1)
                self.data['poses_2d_pred'][cam] = (np.transpose(self.data['poses_2d_pred'][cam].reshape(
                    -1, 16, 2), (0, 2, 1)) - self.add_data['root_joint'][cam]).reshape(-1, 32)
                self.add_data['poses_2d_pred_norm'][cam] = np.linalg.norm(
                    self.data['poses_2d_pred'][cam], ord=2, axis=1, keepdims=True)
                self.data['poses_2d_pred'][cam] /= self.add_data['poses_2d_pred_norm'][cam]
                # 추가
                self.add_data['root_joint_gt'][cam] = np.transpose(
                    self.data['poses_2d_annot'][cam].reshape(-1, 16, 2), (0, 2, 1))[:, :, [0]]
                self.data['poses_2d_annot'][cam] = (np.transpose(self.data['poses_2d_annot'][cam].reshape(
                    -1, 16, 2), (0, 2, 1)) - self.add_data['root_joint_gt'][cam]).reshape(-1, 32)
                self.add_data['poses_2d_pred_norm_gt'][cam] = np.linalg.norm(
                    self.data['poses_2d_annot'][cam], ord=2, axis=1, keepdims=True)
    ...
    def __getitem__(self, idx):
        ...
        for c_idx, cam in enumerate(cams):
            ...
            # gt -----------------------------------------------------
            p2d_gt = torch.Tensor(self.data['poses_2d_annot'][cam][idx].astype(
                'float32')).cuda()   # cuda 있을 필요있나?
            sample['cam' + str(c_idx) + '_2dgt'] = p2d_gt
...
class Dataset3DPW(Dataset):
    ...
    def __init__(self, fname, normalize_2d=True):
        ...
        self.data['confidences']['cam0'] = self.data['confidences']['cam0'].reshape(
            -1, 16)
        ...
        for cam in cams:
            ...
            if normalize_2d:
                self.add_data['root_joint'][cam] = np.transpose(
                    self.data['p2d_pred'][cam], (0, 2, 1))[:, :, [0]]
    ...
    def __getitem__(self, idx):
        ...
        for c_idx, cam in enumerate(cams):
            p2d = torch.Tensor(
                self.data['p2d_pred'][cam][idx].astype('float32')).cuda()
            sample['cam' + str(c_idx)] = p2d
